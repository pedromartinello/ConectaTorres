    ...
    <article className="cartao cartao-prestador">
      <div className="cartao-topo">
        <div className="prestador-identidade">
          ...
          <div><span className="texto-suave texto-pequeno">{perfil.cidade ? `${perfil.cidade}/${perfil.estado || 'RS'}` : 'Região não informada'}</span><h3>{prestador.nome}</h3><p className="texto-suave sem-margem">{perfil.titulo || 'Prestador de serviços'}</p></div>
        </div>
      ...
      <p>{perfil.descricao ? `${perfil.descricao.slice(0, 150)}${perfil.descricao.length > 150 ? '...' : ''}` : 'Este profissional ainda não adicionou uma descrição.'}</p>
